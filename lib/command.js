...
function onError(err) {
    return ConsoleGrid.Style.red(err.message || 'Unknown error');
}
...
async function runGit(config, group, cb) {
    ...
    const runItem = async (item) => {
        ...
        const git = simpleGit(path.resolve(item.dir, '../'), {
            binary: 'git',
            config: [`user.name=${config.name}`, `user.email=${config.email}`]
        });
        await git.init().addRemote('origin', item.url).catch(onError);
        return cb(git, item);
    }
    ...
    program
        ...
        .action((group) => {
            return runGit(config, group, async (git, item) => {
                return git.clone(item.url).catch(onError);
            })
        });
}
